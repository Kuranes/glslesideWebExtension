...
// This library only needs one helper functions:
//
...
const Split = (ids, options = {}) => {
    ...
    return {
        ...
        destroy: destroy,
        isMinimized: function(indexMin) {
            var sizesSplit = this.getSizes();
            return sizesSplit[indexMin] === 2;
        },
        toggle: function(indexToToggle, forceShow) {
            var sizesSplit = this.getSizes();
            var defaultSizeSplit = indexToToggle === 0 ? [20, 80] : [80, 20];
            var minSizeSplit = indexToToggle === 0 ? [2, 98] : [98, 2];

            // it's hidden, we show, unles forced not to
            if ((forceShow = undefined || forceShow === true) && sizesSplit[indexToToggle] === 2) {
                if (this.splitTreePrevSizes) {
                    this.setSizes(this.splitTreePrevSizes);
                } else {
                    this.setSizes(defaultSizeSplit);
                }
            } else if ((forceShow = undefined || forceShow === false)) {
                // it's visible, we hide, unless already hidden
                if (sizesSplit[indexToToggle] !== 2) {
                    this.splitTreePrevSizes = sizesSplit;
                    this.setSizes(minSizeSplit);
                }
            }
        }
    };
